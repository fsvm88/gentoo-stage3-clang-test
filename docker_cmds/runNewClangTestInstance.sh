...
docker run -t -i -rm -v "${PORTDIR}":/portage/gentoo -v "${PORTDIR}":/portage/distfiles -v "${I_OVERLAY}":/portage/local_overlay -v "${I_LOGDIR}":/portage/logs -v "${I_TMPDIR}":/portage/tmp fsvm88/gentoo-stage3-clang-test /bin/bash
